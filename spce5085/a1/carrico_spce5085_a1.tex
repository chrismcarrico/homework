...
\solution{
$angular\ velocity = \omega = \sqrt{\frac{GM}{r^3}} $ \\
$orbital\ radius = r = R_{Earth} + h = 6378.137\ km + 350\ km = 6728.137\ km $ \\
$\omega = \sqrt{\frac{3.986004418e5\ km^3/s^2}{(6728.137\ km)^3}} = 0.001008650132925822\ rad/s$ \\
\ans{angular\ velocity = 0.001008650132925822 rad/s}
}

\subproblem{b.}{The orbital period in minutes.}

\solution{
$orbital\ period = T = \frac{2\pi}{\omega} = \frac{2\pi}{0.001008650132925822\ rad/s} = 6227.5\ s $ \\
...
\solution{
$orbital\ velocity = v = \omega r = 0.001008650132925822\ rad/s * 6728.137\ km = 6.786\ km/s $ \\
$v = 6.786\ km/s = 6786\ m/s$ \\
\ans{orbital\ velocity = 6786 m/s}
}
...
$ r_a = R_{Earth} + altiude\ apogee = 6378.137\ km + 5000\ km = 11378.136\ km $ \\
$ r_p = R_{Earth} + altiude\ perigee = 6378.137\ km + 800\ km = 7178.137\ km $ \\
$ e = \frac{11378.136\ km - 7178.137\ km}{11378.136\ km + 7178.137\ km} = 0.22633 $ \\
\ans{eccentricity = 0.22633}
}

\problem{2.11}{A GEO satellite is located at a longitude of 109\degree West. The satellite broadcasts television programming to the continental United States}
\solution{
  The following answers are all derived using the following equations and equation 2.41 from the textbook: \\
  $central\ angle = \gamma = cos(lat_{earth})cos(lon_{satellite} - lon_{earth})$ \\
  $elevation\ angle = tan^{-1}(\frac{6.61073 - cos(\gamma)}{sin(\gamma)}) - \gamma$, where $6.61073 = \frac{R_{GEO}}{R_{Earth}}$ \\
  $intermediate\ angle = tan^{-1}(\frac{tan(|lon_{satellite} - lon_{earth}|)}{sin(lat_{earth})})$ \\
  ...
  $azimuth = 180\degree - intermediate\ angle$, \\
  $azimuth = 180\degree + intermediate\ angle$, \\
  $azimuth = intermediate\ angle$, or \\
  $azimuth = 360\degree - intermediate\ angle$.
...
\solution{
\ans{elevation\ angle = 37.474\degree, azimuth\ angle = 137.993\degree}
}
...
\solution{
\ans{elevation\ angle = 37.067\degree, azimuth\ angle = 180.00\degree}
}
...
\solution{
\ans{elevation\ angle = 49.307\degree, azimuth\ angle = 195.814\degree}
}
...
\solution{
  ...
  Rio de Janeiro, Brazil: \ans{elevation\ angle = 59.3264\degree, azimuth\ angle = 328.9903\degree} \\
  Santiago, Chile: \ans{elevation\ angle = 31.8804\degree, azimuth\ angle = 302.6809\degree}
}

\problem{3.2}{A three-stabilized satellite has two solar arrays. Each array has an area of 5 m$^2$. At the beginning of life each of the solar sails generate 4kW of electrical power, 
which falls to 3.6 kW at the end of the satellite's useful lifetime. 1kW of power is needed for housekeeping and 3kW is devoted to the telecommunication system}
\subproblem{a.}{Calculate the efficiency of the solar cells at the beginning of life. Assume an incident solar power of 1.36 Kw/m$^2$, and noraml incidence of sunlight on the sails.}
\solution{
  $Total\ power\ generated\ by\ satellite\ at\ BOL = 2 \times 4\ kW = 8\ kW$ \\
  $Efficiency = \frac{Total\ power}{Incident\ power \times Area} = \frac{8\ kW}{1.36\ kW/m^2 \times 10\ m^2} = \ans{0.588}$
}
\subproblem{b.}{Calculate the efficiency of the solar cells at the end of life.}
\solution{
  $Total power generated by satellite at EOL = 2 \times 3.6\ kW = 7.2\ kW$ \\
  $Efficiency = \frac{Total\ power}{Incident\ power \times Area} = \frac{7.2\ kW}{1.36\ kW/m^2 \times 10\ m^2} = \ans{0.529}$
}
\subproblem{c.}{If one of the solar sails fails and produces no power, how much power is avalailable to the telecommunication system.}
\solution{At beginning of life, if one solar sail fails, the total power generated is 4kw. \\
Power for housekeeping doesn't change, so the power available for the telecommunication system is $4\ kW - 1\ kW = \ans{3\ kW}$ \\
...
$Total\ Power = satellite\ requirement + batteries = 2.5\ kW + 1.25 \times 2.5\ kW =\ans{5.625\ kW}$}
\subproblem{b.}{The solar cells supply current at 50 V. How much current do they deliver?}
\solution{$Current = I = \frac{p}{v} = \frac{5.625\ kW}{50\ V} = \ans{112.5\ A}$}
...
\solution{$Ampere\ hours = I \times hours = 112.5\ A \times \frac{46\ min}{60\ min/h} = \ans{86.25\ ampere\ hours}$}
...
\solution{
  $wavelength = \lambda = \frac{c}{f} = \frac{299,792,458\ m/s}{12.2\ GHz} = \ans{0.02457\ m}$
}
\subproblem{b.}{Calculate the gain of the antenna in decibels using the most accurate formula.}
\solution{
  $Area_{Antenna} = A = \pi*0.3^2 = 0.2827\ m^2$ \\
  $Gain = G = \eta_{A}4\pi\frac{A}{\lambda^{2}} = 0.7 * 4\pi\frac{0.2827\ m^2}{(0.02457\ m)^{2}} = 4118.8743$ \\
  $Gain\ dB = 10log_{10}(Gain) = 10log_{10}(4118.8743) = \ans{36.1477\ dB}$
}
...
\solution{
  $Gain\ \approx \frac{33000}{\theta_{3dB}^2} = \frac{33000}{(2.867)^2} = 4015.1207$ \\
  $Gain\ dB = 10log_{10}(Gain) = 10log_{10}(4015.1207) = \ans{36.0369\ dB}$ \\
  This method of approximating gain based on beamwidth is less accurate than the method used in part (b) because it's based on a generic constant, 33000.
}
