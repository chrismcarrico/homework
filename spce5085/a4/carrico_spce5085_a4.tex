...
\solution{There is a peak of attenuation at 22 GHz because of a property of water molecules. This effect is strongest at 22 GHz and weaker at 30 GHz. This effect is still dominant when attenuation is at the 1 -2 dB level, because the attenuation contributed by the molecules is greater than the attenuation contributed by rain.}
...
\solution{Since the earth station is at sea level, the rain height above the station equals the melting layer height, $h = 2.5$ km. The slant path length through the rain is
\[
L = \frac{h}{\sin(\theta)} = \frac{2.5 \text{ km}}{\sin(30\circ)} = \frac{2.5 \text{ km}}{0.5} = \ans{5.00 \text{ km}}
\]}
\subproblem{b}{The path attenuation for a specific attenuation of 1.5 dB/km.}
\solution{The path attenuation is the product of the specific attenuation and the path length through the rain found in part (a):
\[
A = \gamma L = (1.5 \text{ dB/km})(5.00 \text{ km}) = \ans{7.50 \text{ dB}}
\]}
...
\solution{
\begin{center}
\includegraphics[width=0.8\textwidth]{doppler_shift_vs_altitude.png}
\end{center}
}
\end{document}
